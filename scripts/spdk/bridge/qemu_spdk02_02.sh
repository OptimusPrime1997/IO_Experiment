#/bin/bash
taskset -c 11,13 qemu-system-x86_64 \
  --enable-kvm \
  -cpu host \
  -smp 2 \
  -m 2G \
  ...
  -vnc :0 \
  -net nic,model=vmxnet3,name=nic0204,vectors=0 -net tap,ifname=tap0202,script=/etc/qemu-ifup-nat,downscript=/etc/qemu-ifdown-nat \
  -name vm0 &
